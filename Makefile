...
RUN_ARGS=-v ~/.gcpcreds/:/root/.gcpcreds -v /var/run/docker.sock:/var/run/docker.sock -v `pwd`:/problem-worker --restart=always --detach
...
run:
	docker run $(RUN_ARGS) -e LOGURU_LEVEL=INFO $(TAG)
